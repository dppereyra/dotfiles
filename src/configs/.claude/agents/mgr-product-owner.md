...
3. **Perform Task** — the owning lead assigns the implementing agent and whichever of
   ...
   both on the card. The implementing agent does the work and moves the card to Perform Review
   itself when done.
4. **Perform Review** — the assigned qa-reviewer runs the card's test cases against the real
   result. Not satisfied: tells the implementing agent directly what to fix; the implementing
